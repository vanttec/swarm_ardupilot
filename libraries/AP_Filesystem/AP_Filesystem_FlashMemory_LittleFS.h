...
private:
    ...
    // Address where the area dedicated to the file system starts in the
    // flash memory
    ...
    uint32_t lfs_block_to_raw_flash_address(lfs_block_t index);
    bool find_block_size_and_count();
    void flashmem_enable_write();
    bool is_busy();
    bool mount_filesystem();
    uint8_t read_status_register();
    void send_command_addr(uint8_t command, uint32_t addr);
    void wait_until_device_is_ready();
